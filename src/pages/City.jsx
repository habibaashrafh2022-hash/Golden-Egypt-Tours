...
//  Ultra-Pro Pharaonic City Destination Page
//  Route: /city/:cityId
//  Data source: Backend/data/tours.js  (same as Tour.jsx)
//  CSS variables: identical to Tour.jsx — zero conflicts
// ============================================================

import { useState, useEffect, useRef, useCallback } from "react";
import { useParams, useNavigate, Link } from "react-router-dom";
import { getToursByCity, getCityById, getAllCities } from "../data/tours";

// ─────────────────────────────────────────────────────────────
//  CITY META  — hero images + accent per city
// ─────────────────────────────────────────────────────────────
const CITY_META = {
  cairo: {
    heroImages: [
  ...
  alexandria: {
    ...
    desc:"Alexander's legendary city — where Greek philosophy, Egyptian mystique and Roman grandeur fused into something the world has never seen since. Three millennia of layered soul on the sea.",
  ...
  "marsa-alam": {
    ...
    desc:"Egypt's last pristine frontier — wild spinner dolphins, dugongs, virgin coral reefs and a sky so full of stars it looks painted. Only for those who demand the truly extraordinary.",
...
// Tour images by category — used when tour has no images
const CAT_IMG = {
  sightseeing:[
    "https://images.unsplash.com/photo-1568322445389-f64ac2515020?w=600&q=85",
    "https://images.unsplash.com/photo-1601921004897-b7d582836990?w=600&q=85",
    "https://images.unsplash.com/photo-1548574505-5e239809ee19?w=600&q=85",
    "https://images.unsplash.com/photo-1600093463592-8e36ae95ef56?w=600&q=85",
    "https://images.unsplash.com/photo-1553913861-c0fddf2619ee?w=600&q=85",
  ],
  adventure:[
    ...
    "https://images.unsplash.com/photo-1560275619-4cc5a455b0a6?w=600&q=85",
    "https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=600&q=85",
    "https://images.unsplash.com/photo-1527576539890-dfa815648363?w=600&q=85",
  ],
  culture:[
    "https://images.unsplash.com/photo-1544967082-d9d25d867d66?w=600&q=85",
    "https://images.unsplash.com/photo-1572977078501-8c8f3e4fdd34?w=600&q=85",
    "https://images.unsplash.com/photo-1559592413-7cec4d0cae2b?w=600&q=85",
    "https://images.unsplash.com/photo-1594736797933-d0501ba2fe65?w=600&q=85",
  ],
  cruise:[
    "https://images.unsplash.com/photo-1605281317010-fe5ffe798166?w=600&q=85",
    "https://images.unsplash.com/photo-1553913861-c0fddf2619ee?w=600&q=85",
    "https://images.unsplash.com/photo-1548574505-5e239809ee19?w=600&q=85",
  ],
  experience:[
    "https://images.unsplash.com/photo-1553913861-c0fddf2619ee?w=600&q=85",
    "https://images.unsplash.com/photo-1605281317010-fe5ffe798166?w=600&q=85",
    "https://images.unsplash.com/photo-1533130061792-64b345e4a833?w=600&q=85",
  ],
  "day-trip":[
    "https://images.unsplash.com/photo-1568322445389-f64ac2515020?w=600&q=85",
    "https://images.unsplash.com/photo-1527576539890-dfa815648363?w=600&q=85",
...
function tourImg(tour, idx = 0) {
  if (tour.images?.length) return tour.images[0];
  const arr = CAT_IMG[tour.category] || CAT_IMG[tour.type] || CAT_IMG.sightseeing;
  return arr[idx % arr.length];
}

// ─────────────────────────────────────────────────────────────
//  GLOBAL STYLES  (same CSS vars as Tour.jsx, no conflicts)
// ─────────────────────────────────────────────────────────────
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .cp-stat{padding:18px 28px;text-align:center;border-right:1px solid rgba(201,168,76,.12);
      transition:background .2s}
    .cp-stat:last-child{border-right:none}
    .cp-stat:hover{background:rgba(201,168,76,.07)}
    .cp-stat-n{font-family:var(--fd);font-size:21px;font-weight:700;line-height:1;
      color:var(--ca,var(--gold))}
    .cp-stat-l{font-family:var(--ft);font-size:8px;letter-spacing:2px;color:var(--muted);
    ...
    .cp-city-tab .cnt{font-family:var(--ft);font-size:7px;background:var(--gold-dim);
      border:1px solid rgba(201,168,76,.2);border-radius:8px;padding:1px 6px;color:rgba(201,168,76,.6)}
    ...
    .cp-ftag .fc{font-family:var(--ft);font-size:7px;background:rgba(255,255,255,.05);
    ...
    .cp-sh-lbl{font-family:var(--ft);font-size:10px;letter-spacing:3px;color:var(--gold);
    ...
    .cp-cimg-ov{position:absolute;inset:0;
      ...
      transition:all .3s}
    .cp-card:hover .cp-cimg-ov{background:linear-gradient(to top,rgba(7,5,13,.88) 0%,rgba(7,5,13,.08) 50%,transparent 100%)}
    .cp-cshine{position:absolute;inset:0;opacity:0;
      background:radial-gradient(ellipse at 50% 0%,rgba(201,168,76,.12),transparent 65%);transition:opacity .3s}
    .cp-card:hover .cp-cshine{opacity:1}

    .cp-cbadge{position:absolute;top:12px;left:12px;border-radius:4px;padding:4px 11px;
      font-family:var(--ft);font-size:7px;letter-spacing:2px;text-transform:uppercase;font-weight:700}
    .cp-cbadge.sell{background:rgba(201,168,76,.9);color:#07050D}
    .cp-cbadge.top{background:rgba(200,50,50,.85);color:#fff}
    .cp-cbadge.new{background:rgba(42,160,100,.85);color:#fff}
    .cp-cbadge.uni{background:rgba(80,100,200,.85);color:#fff}
    ...
    .cp-cgrp{position:absolute;bottom:12px;right:12px;background:rgba(7,5,13,.82);
      backdrop-filter:blur(10px);border:1px solid rgba(201,168,76,.15);border-radius:5px;
      padding:4px 10px;font-family:var(--ft);font-size:7px;letter-spacing:1px;color:var(--muted)}
    ...
    .cp-chip{background:var(--gold-dim);border:1px solid rgba(201,168,76,.18);color:rgba(237,232,217,.65);
      border-radius:10px;padding:2px 9px;font-family:var(--ft);font-size:7px;letter-spacing:1px}
    ...
    .cp-cpop{background:var(--gold-dim);border:1px solid var(--gold-border);border-radius:4px;
      padding:2px 8px;font-family:var(--ft);font-size:6px;letter-spacing:1.5px;color:var(--gold)}
    ...
    .cp-cfrom{font-family:var(--ft);font-size:7px;letter-spacing:2px;color:var(--muted);
    ...
    .cp-oc-t{font-family:var(--ft);font-size:7px;letter-spacing:1px;color:var(--muted);margin-top:2px}
    /* ── FLOATING ── */
    .cp-wa{position:fixed;bottom:28px;right:28px;z-index:900;display:flex;align-items:center;gap:8px;
      ...
      box-shadow:0 8px 32px rgba(37,211,102,.4);text-transform:uppercase;animation:cp-fu .8s 1s both}
    ...
    .cp-f input,.cp-f textarea,.cp-f select{background:var(--surface2);border:1px solid var(--gold-border);
    ...
    .cp-f select option{background:var(--surface2)}
    ...
    .cp-msub:not(:disabled):hover{box-shadow:0 6px 28px rgba(201,168,76,.45)}
    ...
    .cp-mnote{font-family:var(--fb);font-size:13px;color:var(--muted);text-align:center;
      margin-top:10px;font-style:italic}
    .cp-msuc{text-align:center;padding:20px 0}
    .cp-msuc-ic{font-size:64px;margin-bottom:14px;animation:cp-bnc .5s ease}
    @keyframes cp-bnc{0%{transform:scale(0)}70%{transform:scale(1.2)}100%{transform:scale(1)}}
    ...
    /* ── LOADING ── */
    .cp-load{background:var(--obsidian);min-height:100vh;display:flex;
...
function BookingModal({ tour, cityName, onClose }) {
  ...
  const total = (tour?.price ?? 0) * parseInt(f.guests || 1);
  ...
  const submit = async () => {
    ...
    try {
      await fetch("/api/bookings", {
        ...
        body: JSON.stringify({ tourId:tour.id, tourName:tour.name, cityName, ...f, total }),
      });
    } catch(_) {}
      ...
      <div className="cp-m">
        <div className="cp-mhd">
          <div>
            ...
            <div className="cp-mtit">{tour.name}</div>
          </div>
          ...
          {sent ? (
            <div className="cp-msuc">
              ...
              <div className="cp-msuc-s">
                Our team will contact you within 2 hours to finalize details.<br/>
            ...
            <>
              <div className="cp-psum">
                <div>
                  ...
                  <div style={{ fontFamily:"var(--fb)", fontSize:14, color:"var(--muted)", marginTop:2 }}>
                    ${tour.price} × {f.guests} = <strong style={{ color:"var(--gold)" }}>${total}</strong>
                  </div>
                </div>
              ...
              <div className="cp-fg">
                {[
                  { k:"name",   l:"Full Name",       p:"Your full name",   t:"text",   col:true  },
                  { k:"email",  l:"Email Address",    p:"email@example.com",t:"email",  col:false },
                  { k:"phone",  l:"Phone / WhatsApp", p:"+20 xxx xxx xxxx", t:"tel",    col:false },
                  { k:"date",   l:"Tour Date",        p:"",                 t:"date",   col:false },
                  { k:"guests", l:"Guests",           p:"2",                t:"number", col:false },
                ].map(fi => (
                  <div key={fi.k} className={`cp-f${fi.col?" full":""}`}>
                    <label>{fi.l}</label>
...
function TourCard({ tour, index, onBook, onView }) {
  const BDGMAP = {
    bestseller:{ cls:"sell", lbl:"🏆 Best Seller" },
    "top-rated":{ cls:"top",  lbl:"⭐ Top Rated"   },
    new:        { cls:"new",  lbl:"🆕 New"          },
    unique:     { cls:"uni",  lbl:"💎 Unique"        },
  };
  const bdg  = tour.badge ? BDGMAP[tour.badge] : (tour.bestseller ? BDGMAP.bestseller : null);
  const img  = tourImg(tour, index);
  const orig = tour.originalPrice;
  const disc = orig ? Math.round((1 - tour.price/orig)*100) : 0;
  const includes = (tour.includes||[]).slice(0,3).map(i => typeof i==="string"?i:i.label||i.name||"");
  const rating = tour.rating || 4.8;
  const revCnt = tour.reviewCount || (tour.reviews?.length||0);

  return (
    <div className="cp-card" style={{ animationDelay:`${index*0.07}s` }} onClick={() => onView(tour.id)}>
      {/* Image */}
      <div className="cp-cimg">
        <img src={img} alt={tour.name}
          onError={e => { e.target.src=`https://placehold.co/600x210/0C0A14/C9A84C?text=${encodeURIComponent((tour.name||"Tour").slice(0,16))}`; }} />
        <div className="cp-cimg-ov" />
        <div className="cp-cshine" />
        {bdg && <div className={`cp-cbadge ${bdg.cls}`}>{bdg.lbl}</div>}
        {orig && <div className="cp-cdisc">-{disc}%</div>}
        <div className="cp-cdur">⏱ {tour.duration}</div>
        <div className="cp-cgrp">
          {tour.groupType==="private"?"🔒 Private":tour.groupType==="small"?"👥 Small Group":"🌐 Shared"}
        </div>
      </div>
      {/* Body */}
      <div className="cp-cbody">
        <div className="cp-ctitle">{tour.name}</div>
        <div className="cp-cdesc">{tour.description}</div>
        {includes.length > 0 && (
          <div className="cp-chips">
            {includes.map((l,i) => l && <span key={i} className="cp-chip">✓ {l}</span>)}
            {(tour.includes||[]).length>3 && <span className="cp-chip" style={{ opacity:.5 }}>+{(tour.includes||[]).length-3}</span>}
          </div>
        )}
        <div className="cp-crat">
          <span className="cp-cstars">{"★".repeat(Math.floor(rating))}</span>
          <span className="cp-cscore">{rating.toFixed(1)}</span>
          <span className="cp-crevs">({revCnt.toLocaleString()} reviews)</span>
          {revCnt > 2000 && <span className="cp-cpop">🔥 Popular</span>}
        </div>
        <div className="cp-cfoot">
          <div>
            <div className="cp-cfrom">From</div>
            <div className="cp-cprow">
              {orig && <span className="cp-corig">${orig}</span>}
              <span className="cp-cpamt">${tour.price}</span>
...
// ─────────────────────────────────────────────────────────────
const CATS = [
  { id:"all",         lbl:"All Experiences", icon:"✦" },
  { id:"sightseeing", lbl:"Sightseeing",      icon:"🏛️" },
  { id:"adventure",   lbl:"Adventure",        icon:"⚡" },
  { id:"culture",     lbl:"Culture",          icon:"🎭" },
  { id:"cruise",      lbl:"Cruises",          icon:"⛴" },
  { id:"experience",  lbl:"Experiences",      icon:"✨" },
...
export default function CityPage() {
  ...
  const meta = CITY_META[cityId] || CITY_META.cairo;

  // Load from Backend/data/tours.js
  useEffect(() => {
    setLoad(true);
    window.scrollTo({ top:0 });
    setCat("all"); setHero(0);
    try {
      setCity(getCityById(cityId) || { id:cityId, name:cityId });
      setTours(getToursByCity(cityId) || []);
      setAll(getAllCities() || []);
  ...
  const filtered = useCallback(() => {
    let l = [...tours];
    if (cat!=="all") l = l.filter(t => (t.category||t.type) === cat);
    ...
    if (sort==="popular")    l.sort((a,b) => (b.reviewCount||0)-(a.reviewCount||0));
    return l;
  }, [tours, cat, sort])();

  const counts = CATS.reduce((a,c) => {
    a[c.id] = c.id==="all" ? tours.length : tours.filter(t=>(t.category||t.type)===c.id).length;
    return a;
  }, {});

  const bestsellers = tours.filter(t => t.badge==="bestseller" || t.bestseller);
  const cityName    = city?.name || city?.cityName || cityId;
  const minPrice    = tours.length ? Math.min(...tours.map(t=>t.price||9999)) : 0;
  ...
  return (
    <>
      ...
      <div className={`cp-sticky${sticky?" on":""}`}>
        ...
        <span className="cp-sticky-price">From ${minPrice}</span>
      ...
      <section className="cp-hero">
        ...
        <div className="cp-hero-cnt">
          ...
          <span className="cp-city-nm-ar">{meta.glyphs.slice(0,6).join("  ")}</span>
          <div className="cp-city-div">
            <p className="cp-tagline">{meta.tagline}</p>
          </div>
          <p className="cp-city-desc-txt">{meta.desc}</p>
          <div className="cp-stats">
            {[
              { n:`${tours.length}+`,      l:"Tours Available"  },
              { n:city?.stats?.travelers||"5,000+", l:"Happy Travelers" },
              { n:city?.stats?.rating||"4.9 ★",     l:"Avg Rating"     },
              { n:`From $${minPrice}`,               l:"Lowest Price"   },
            ].map((s,i) => (
      ...
      <div className="cp-city-bar">
        {allCities.map(c => {
          const cm = CITY_META[c.id] || {};
          const cnt = (getToursByCity(c.id)||[]).length;
          return (
            <Link key={c.id} to={`/city/${c.id}`}
              className={`cp-city-tab${c.id===cityId?" on":""}`}
              style={ c.id===cityId ? { borderBottomColor:cm.color||"var(--gold)", color:cm.color||"var(--gold)", background:`${cm.color||"#C9A84C"}14` } : {} }>
              {cm.glyph||"✦"} {c.name}
      ...
      {bestsellers.length > 0 && (
        <div className="cp-bs-sec">
          <div className="cp-sh">
            <span className="cp-sh-lbl">🔥 Must-Do Experiences</span>
            <div className="cp-sh-ln" />
            <span className="cp-sh-cnt">{bestsellers.length} tours</span>
          </div>
      ...
      <div className="cp-grid-sec">
        {cat !== "all" && (
          <div className="cp-sh" style={{ marginBottom:24 }}>
            <span className="cp-sh-lbl">
              {CATS.find(c=>c.id===cat)?.icon} {CATS.find(c=>c.id===cat)?.lbl} in {cityName}
            </span>
            <div className="cp-sh-ln" />
          </div>
        )}
        <div className="cp-grid">
          {filtered.length===0
            ? <div className="cp-empty"><div className="cp-empty-ic">𓂀</div><div className="cp-empty-t">No experiences in this category</div></div>
            : filtered.map((t,i) => (
              <TourCard key={t.id} tour={t} index={i}
                onBook={setBook} onView={id => navigate(`/tour/${id}`)} />
            ))
      ...
      <div className="cp-others">
        ...
        <div className="cp-og">
          {allCities.filter(c=>c.id!==cityId).map(c => {
            const cm  = CITY_META[c.id]||{};
            const cnt = (getToursByCity(c.id)||[]).length;
            return (
              <Link key={c.id} to={`/city/${c.id}`} className="cp-oc">
                <img src={cm.heroImages?.[0]||"https://placehold.co/200x120/0C0A14/C9A84C?text=Egypt"}
                  alt={c.name} onError={e=>{e.target.src=`https://placehold.co/200x120/0C0A14/C9A84C?text=${c.name}`;}} />
                <div className="cp-oc-ov" />
                <div style={{ position:"absolute", inset:0, background:`radial-gradient(circle at 0% 100%,${cm.color||"#C9A84C"}22,transparent 65%)` }} />
                <div className="cp-oc-c">
                  <div className="cp-oc-g">{cm.glyph||"𓂀"}</div>
                  <div className="cp-oc-n" style={{ color:cm.color||"var(--gold)" }}>{c.name}</div>
                  <div className="cp-oc-t">{cnt} tours</div>
                </div>
              </Link>
            );
          })}
